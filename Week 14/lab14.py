...
class OrderSystem:
    ...
    def cancel_Order(self):
        try:
            ...
            print(f'Error: {self.process.name} cannot cancel your order in  {self.process.current_state} state')
            
    def back(self):
        try:
                self.process.back()
        except  InvalidStateTransition as err:
            print(f'Error: {self.process.name} cannot return to checkout in  {self.process.current_state} state')
