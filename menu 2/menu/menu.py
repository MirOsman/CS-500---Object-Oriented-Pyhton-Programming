...
class Holiday:
    ...
    def __str__(self):
        return self.name + ":" + str([booking.balloon_twister.name for booking in self.bookings])

    def __repr__(self):
        return self.__str__()

class Booking:
    def __init__(self, customer : str, balloon_twister, holiday):
...
class Scheduler:  
    ...
    def schedule(self, customer, holiday):
        should_add_to_waiting = True
        for balloon_twister in self.balloon_twisters:
            if not balloon_twister.availability(holiday):
                booking = Booking(customer, balloon_twister, holiday )
                balloon_twister.book_holiday(holiday)
                holiday.add_booking(booking)
                should_add_to_waiting = False
    ...
    def cancel(self, customer, holiday):
        ...
        print("Sorry, We were unable to find any booking for " + customer +  "  on  "  + holiday.name)
    ...
    def status(self, name):
        ...
        print("We could not find any such data " + name)

    def signup(self, name):
       balloon_twister = BalloonTwister(name)
       self.add_balloon_twister(balloon_twister)
       print("Succesfully Added " + name + " to balloon twisters")

    def dropout(self, name):
        for balloon_twister in self.balloon_twisters:
            if balloon_twister.name == name:
               self.cancel_balloon_twister(balloon_twister)
                    ...
                    for booking in holiday.bookings:
                       if booking.balloon_twister == balloon_twister:
                           ...
                           if self.waiting_list:
                            ...
                            print("We have Rescheduled " + customer + " from waiting list")

               print("We have dropped " + name + " from balloon twisters")
               return

        print("We could not find any such data " + name)

    def load_data(self):
        [bt, holiday, balloon_twister_schedule] = DB().load_data()
        
        for btwister in bt:
            self.signup(btwister)

        for hd in holiday:
            holiday = Holiday(hd)
            self.add_holiday(holiday)

        for row in balloon_twister_schedule:
       ...
       while True:
        ...
        if command == "quit":
            ...
            try:
                ...
                print("Sorry, We could not find any holiday")
        elif command == "status":
            name = input("Name: ")
            self.status(name)
        elif command == "signup":
